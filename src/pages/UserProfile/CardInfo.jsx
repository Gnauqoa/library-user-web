...
const CardInfo = ({ getBorrowingRequest, getBorrowedRequest }) => {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      />
      {!getBorrowingRequest.isFetched ? (
        <CircularProgress />
      ) : (
        <InfoItem
          icon={IconBook}
          info={`Borrowing book: ${getBorrowingRequest.response.data.total_items}`}
        />
      )}
      {!getBorrowedRequest.isFetched ? (
        <CircularProgress />
      ) : (
        <InfoItem
          icon={IconBook2}
          info={`Borrowed book: ${getBorrowedRequest.response.data.total_items}`}
        />
      )}
